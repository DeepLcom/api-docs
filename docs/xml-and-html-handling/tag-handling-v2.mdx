...
Tag handling v2 uses a word-based pipeline that balances both translation quality and formatting. Sentences are now be translated more naturally without being constrained by tag placement in the source text. It also handles edge cases around tag preservation, DOM hierarchy, and character escaping.

V2 enforces strict XML parsing and will return an error if invalid XML is submitted.

## Usage Examples

### Basic XML Translation with v2

<Tabs>
  <Tab title="Simple Markup">
    ```text Parameters
...
### HTML Translation with v2

<Card title="HTML translation with tag handling v2">
  ```text Parameters
...
Tag handling v2 is currently only compatible with the `quality-optimized` model type. The table below shows model compatibility:
...
As a result of improvements, tag handling is expected to give different results between v1 and v2. 

Thoroughly test your system and verify any differences in output before switching to `v2` in production.

### 1. Test Your Content Types
