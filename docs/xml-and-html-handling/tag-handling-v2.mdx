...
Tag handling v2 uses an algorithm that balances both translation quality and formatting. Sentences are now be translated more naturally without being constrained by tag placement in the source text. It also handles edge cases around tag preservation, DOM hierarchy, and character escaping.
