...
[Custom rules](/api-reference/custom-rules) make it possible to create a set of rules to apply when translating text, ensuring that translations adhere to company guidelines effortlessly. By allowing users to create tailored rules, this feature streamlines the writing process, eliminating the need for time-consuming manual edits to align with company policies.

These rules make changes to the text according to the selected formatting and spelling conventions, including rules around punctuation, date format, style, and more.