...
You can learn more about the `context` parameter in the [text translation API reference](/api-reference/translate#request-body-descriptions).
...
When calling the `translate` endpoint for [text translation](/api-reference/translate), it’s possible to send multiple text parameters in a single request.
...
The DeepL engine also utilizes the broader context of a document when [translating documents](/api-reference/document).
...
[Style rules](/api-reference/style-rules) make it possible to create a set of rules to apply when translating text, ensuring that translations adhere to company guidelines effortlessly. By allowing users to create tailored rules, this feature streamlines the writing process, eliminating the need for time-consuming manual edits to align with company policies.

These rules make changes to the text according to the selected formatting and spelling conventions, including rules around punctuation, date format, style, and more. You can also write your own rules to define your own custom instructions for how text should be translated or improved