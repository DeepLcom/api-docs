...
- **Model behavior**: When `custom_instructions` are included and no `model_type` is specified, the API uses `quality_optimized` models. You can also explicitly set `model_type` to `latency_optimized` or `quality_optimized`.
