...
- Improved overall quality of the models used in the write API
- Expanded language support to Japanese (`JA`), Korean (`KO`) and (simplified) Chinese (`ZH` or `zh-Hans`)
- Expanded support of writing styles and tones to Spanish (`ES`), Italian (`IT`), French (`FR`), Portuguese (`pt-PT`) and Brazilian Portuguese (`pt-BR`)
- Allow sending texts in multiple source languages in the same request when no `target_lang` is set (similar to translate)
