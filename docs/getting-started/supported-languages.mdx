...
- `FI` - Finnish
- `FR` - French
- `HE` - Hebrew _(next-gen models only)_
- `HU` - Hungarian
- `ID` - Indonesian
- `IT` - Italian
- `JA` - Japanese
...
- `SL` - Slovenian
- `SV` - Swedish
- `TH` - Thai _(next-gen models only)_
- `TR` - Turkish
- `UK` - Ukrainian
- `VI` - Vietnamese _(next-gen models only)_
...
<Info>
  Beta languages can be enabled by adding `enable_beta_languages: true` (or 1 in url-encoded requests) to the body of your translation request. Note that these languages do not support glossaries or formality yet.

  The latest versions of official client libraries have an option for extra body/request parameters to allow passing this option; see the client library readme for more information.
