...
<Steps>
  <Step title="Sign up for the API" titleSize="h3">
    ...
    <Tip>
      if you already have a DeepL Translator account, you will need to log out and create a new one for the DeepL API.
    </Tip>
  </Step>
  <Step title="Step 2: Test your API key with a request" titleSize="h3">
    Next, try making a translation request to the API using cURL or six popular programming languages. Choose your favorite, making sure to replace`{YOUR_API_KEY}`with your own API key, which you can find [here](https://www.deepl.com/en/your-account/keys).
    You can also try out translation requests [in our playground](https://developers.deepl.com/api-reference/translate/request-translation?playground=open).

    <Tip>
      If you chose a free API plan and you are using cURL or HTTP requests, replace `https://api.deepl.com` with `https://api-free.deepl.com`.
    </Tip>
    <Tabs>
      <Tab title="Curl">
        Open a terminal.
  ...
  <Step title="Step 3: Keep building with our client libraries and how-to guides" titleSize="h3">
    [Our client libraries](/api-reference/client-libraries) let you use the API with popular programming languages (Python, JavaScript, .NET, PHP, Java, Ruby).  You can also click [here](/docs/learning-how-tos/examples-and-guides) to find more examples and guides.
  </Step>
</Steps>
