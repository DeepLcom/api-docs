...
<Info>
  This page describes the differences between `v2` and `v3` glossaries, which are accessed via `/v2` and `/v3` endpoints. At present, the `/v3` endpoint is only used for working with `v3` glossaries.
</Info>
