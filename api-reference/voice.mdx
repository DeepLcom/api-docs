...
<Info>
    DeepL Voice API is now **available on request** for customers with a DeepL API Enterprise subscription
    via the v3 API endpoint. The API provides real-time speech transcription, translation, and translated
    speech. The supported scope of functionality is covered in this documentation page.
    ...
    DeepL Voice API with the following applicable additions to the [Terms and Conditions, the Service Specification
    and the Data Processing Agreement](/api-reference/voice/deepl-voice-api-service-specification-updates)
...
* <Badge color="green">closed beta</Badge> Receive translated speech in multiple target languages
...
Continuously send audio using [source media chunk](/api-reference/voice/websocket-streaming#sourcemediachunk) messages.
Use smaller chunks to get a lower latency. The recommended chunk duration is 50-250 milliseconds for optimal performance.
Send an [end of source media](/api-reference/voice/websocket-streaming#endofsourcemedia) message to signal that the
...
[source transcript updates](/api-reference/voice/websocket-streaming#sourcetranscriptupdate) and
[target transcript updates](/api-reference/voice/websocket-streaming#targettranscriptupdate):
...
[target media chunks](/api-reference/voice/websocket-streaming#targetmediachunk). To save on bandwidth,
