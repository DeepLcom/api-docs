...
<Warning>
  The WebSocket only accepts TEXT frames when using JSON format, and only BINARY frames when using MessagePack format. Sending the wrong frame type will result in connection errors.
</Warning>
...
Your application should merge concluded segments into the final transcript while displaying tentative
